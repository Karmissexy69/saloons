...
import type { CustomerVoucher, VoucherCatalogItem } from "../lib/types";
...
export function VouchersPage() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      const [voucherData, catalogData] = await Promise.all([voucherPromise, catalogPromise, getCustomerMe()]);
  ...
  const available = useMemo(
    () => vouchers.filter((voucher) => voucher.status === "AVAILABLE"),
    [vouchers]
  );
    ...
    <section className="ve-account-section">
      ...
      <div className="ve-panel ve-panel-elevated ve-balance-strip">
        <div>
          ...
          <h2 className="ve-display-sm">{formatPoints(session?.customer.pointsBalance)}</h2>
        </div>
      </div>
      ...
      <section className="ve-stack-lg">
        ...
        {loading ? (
          ...
          <div className="ve-panel ve-panel-muted">
            <p className="ve-panel-title">No available vouchers yet</p>
            <p className="ve-panel-copy">Redeemed rewards will appear here once created in your customer wallet.</p>
          </div>
        ) : (
          <div className="ve-card-grid">
            {available.map((voucher) => (
              <article key={voucher.id} className="ve-panel ve-panel-elevated">
                ...
                <div className="ve-chip-row">
                  {voucher.discountValue ? <span className="ve-chip">{formatCurrency(voucher.discountValue)}</span> : null}
          ...
          <div className="ve-card-grid">
            {catalog.map((item) => (
              <article key={item.catalogId} className="ve-panel ve-panel-elevated">
                <p className="ve-card-kicker">{item.voucherType}</p>
                ...
                <button
                  ...
                  disabled={redeemingId === item.catalogId}
                >
                  {redeemingId === item.catalogId ? "Redeeming..." : "Redeem"}
                </button>
              </article>
            ))}
          </div>
